...
def get_fits_by_sliding_windows(birdeye_binary, line_lt, line_rt, n_windows=9, verbose=False):
    if birdeye_binary is None or birdeye_binary.size == 0:
        raise ValueError("birdeye_binary is empty or None")
    ...
    detected = True
    if len(line_lt.all_x) == 0 or len(line_lt.all_y) == 0:
    ...
    if len(line_rt.all_x) == 0 or len(line_rt.all_y) == 0:
